...
function checkData(data) {
 if(Object.keys(data).length>0) {return data}// Якщо об'єкт не пустий повертаємо дані
  else{const newErr=new Error( "Об'єкт пустий"); return newErr}// Інакше створюємо помилку,в якості тексту помилки ми використовуємо рядок "Об'єкт пустий".
...
function parseJson(jsonStr) {
  try{const obj=JSON.parse(jsonStr);
    return obj;// Спроба розпарсити JSON-рядок.
}catch(err){
const newError=new Error(' рядок має невірний формат!',{cause:err,});// Якщо рядок має невірний формат, виникне помилка, яку ми обробляємо у блоку catch.
 console.log(newError);}// Якщо виникла помилка, повертаємо її повідомлення.
}
 ...
 */
function getAge(age){
 try{if(age<0){throw new Error('Вік не може бути менше 0!')}
  else{return `Вік користувача: ${age}`}}catch(err){err.name= "AgeError";
   const nam=err.name;
return {err,nam}}; // Спроба отримати вік користувача.
...
function getBookById(books, id) {
 try{
  ...
  if(books[i].id===id){
    console.log(`Книга з ID ${id} знайдена!Це ${books[i].title}`);
     b=true}
}
  if(b===false){throw new Error(`Книга з ID ${id} не знайдена!`)}
  }catch(Error){console.log(Error.message)}
 // Спроба знай ти книгу по ID та записати в змінну book.
...
function decodeURIComponentWrapper(encodedString) {
  try{
    let decodeURL=decodeURIComponent(encodedString);
    if(decodeURL!==undefined){return decodeURL};
  }catch(err){return  `${err} неправильний URI формат`}// Спроба декодувати рядок
...
function findEvenNumber(numbers) {
  ...
  let j=0;// Створюємо змінну evenNumber без значення
  console.log(numbers);
  try{for(let i=0;i<numbers.length;i++)
  { if((numbers[i]%2===0)&&(j===0)){evenNumber=numbers[i];
      j=1;
    }}
  if(j===0){throw new Error('У масиві немає чисел, що діляться на 2 без остачі!')}}// Шукаємо перше число, що ділиться на 2 без остачі, та записуємо в нашу змінну.
  catch(error){return error.message};// Якщо такого числа немає, кидаємо помилку з повідомлення У масиві немає чисел, що діляться на 2 без остачі!.
  if(j===1){return evenNumber}// Якщо число знайдено повертаємо його
...
function validateUser(user) {
 try{if(user===undefined){const error=new Error("Об'єкт користувача не вказано!")}// Перевіряємо, чи існує об'єкт користувача,якщо ні викидуємо помилку з повідомленням "Об'єкт користувача не вказано!".
  else if(user.hasOwnProperty('name')===false){throw new Error("Ім'я користувача не вказано!",{cause:user})}// Перевіряємо, чи існує ім'я користувача,якщо ні викидуємо помилку з повідомленням "Ім'я користувача не вказано!", а як причину вказуємо об'єкт user.
  else if(user.hasOwnProperty('email')===false){throw new Error("Email користувача не вказано!",{cause:user})} // Перевіряємо, чи існує email користувача,якщо ні викидуємо помилку з повідомленням "Email користувача не вказано!", а як причину вказуємо об'єкт user.
  else{console.log("Об'єкт користувача відповідає всім вимогам.")}}// Якщо всі перевірки пройдено успішно виводимо повідомлення "Об'єкт користувача відповідає всім вимогам."
  catch(error){
...
function calculateSquareRoot(number) {
  try{ if(isNaN(number)){throw new TypeError("Аргумент має бути числом!")}// Перевіряємо, чи аргумент є числом, якщо ні викидуємо помилку про невірний тип даних з повідомленням Аргумент має бути числом!".
  else if(number<0){throw new RangeError(" Число не повинно бути від'ємним!")}// Перевіряємо, чи число не від'ємне, якщо ні викидуємо помилку про тип недопустимий діапазон з повідомленням Число не повинно бути від'ємним!".
 else{return Math.sqrt(number)} // Повертаємо корінь квадратний з вхідного значення
  }catch(err){return err.message}// Повертаємо повідомлення про помилку.
}
...
function processData(data) {
 ...
 try{for(let index=0;index<data.length;index++){
  if(isNaN(data[index])){throwError(`Елемент з індексом ${index} має бути числом!`)}
 }// Для кожного елемента в масиві
}// Перевіряємо, чи елемент є числомs
 catch(error){console.log(error.stack)} // Якщо елемент не є числом, кидаємо помилку невірного типу даних з повідомленням `Елемент з індексом ${index} має бути числом!`
  finally{return data}// Повертаємо рядок "Дані успішно оброблені"
...
function evaluateExpression(expression) {
  ...
  if(result===undefined){throw new EvalError()}
  return result
}// Повертаємо результат розрахунку
  catch(err){console.log(err)}// Якщо була виявлена помилка повертаємо помилку при виконанні функції eval
}
